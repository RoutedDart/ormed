...
void main() async {
  ...
  void display() {
    ...
    for (var j = 0; j < 7; j++) {
      ...
      terminal.fillArea(Cell(content: ' ', style: Style(bg: rowColors[0][j])), bar);
    }

    // Mid Row
    for (var j = 0; j < 7; j++) {
      final bar = rect(j * barWidth, topRowHeight, (j == 6) ? width - j * barWidth : barWidth, midRowHeight);
      terminal.fillArea(Cell(content: ' ', style: Style(bg: rowColors[1][j])), bar);
    }
    ...
    for (var j = 0; j < 6; j++) {
      ...
      terminal.fillArea(Cell(content: ' ', style: Style(bg: rowColors[2][j])), bar);
    }
