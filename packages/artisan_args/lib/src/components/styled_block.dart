...
class StyledBlockComponent extends CliComponent {
  ...
  RenderResult build(ComponentContext context) {
    ...
    final blockColor = switch (blockStyle) {
      BlockStyleType.info =>
        (String s) => context.newStyle().foreground(Colors.info).render(s),
      BlockStyleType.success =>
        (String s) => context.newStyle().foreground(Colors.success).render(s),
      BlockStyleType.warning =>
        (String s) => context.newStyle().foreground(Colors.warning).render(s),
      BlockStyleType.error =>
        (String s) => context.newStyle().foreground(Colors.error).render(s),
      BlockStyleType.note => (String s) => context.newStyle().dim().render(s),
    };
...
class CommentComponent extends CliComponent {
  ...
  RenderResult build(ComponentContext context) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      buffer.write(context.newStyle().dim().render('// ${lines[i]}'));
    }
