...
/// Converts a [Style] to respect the given terminal color [profile].
///
/// Upstream: `third_party/ultraviolet/cell.go` (`ConvertStyle`).
Style convertStyle(Style style, cp.Profile profile) {
  switch (profile) {
...
///
/// Upstream: `third_party/ultraviolet/cell.go` (`Style.String`).
String styleToSgr(Style style) {
...
String styleDiff(Style? from, Style? to) {
  if (from == null && to == null) return '';
  if (from != null && to != null && from == to) return '';
  if (from == null) return styleToSgr(to ?? const Style());
