...
void main() {
  test('TerminalThemeState updates from TerminalColorMsg background', () {
    var s = const TerminalThemeState();
    s = s.update(
      const TerminalColorMsg(kind: TerminalColorKind.background, hex: '#ffffff'),
    );
    expect(s.backgroundHex, '#ffffff');
    expect(s.hasDarkBackground, false);

    s = s.update(
      const TerminalColorMsg(kind: TerminalColorKind.background, hex: '#000000'),
    );
  ...
  test('TerminalThemeState keeps dark flag on unparseable hex', () {
    ...
    s = s.update(
      const TerminalColorMsg(kind: TerminalColorKind.background, hex: 'rgb:aa/bb/cc'),
    );
