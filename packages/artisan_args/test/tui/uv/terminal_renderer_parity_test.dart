...
void main() {
  test('TerminalRenderer parity: simple renderer output', () {
    const w = 5;
    const h = 3;
    final out = _TestSink();
  ...
  test('TerminalRenderer parity: inline renderer output', () {
    final out = _TestSink();
    final r = TerminalRenderer(
      out,
      env: const ['TERM=xterm-256color', 'COLORTERM=truecolor'],
  ...
  test('TerminalRenderer parity: color profile setter', () {
    final out = _TestSink();
    ...
    buf.setCell(
      ...
      Cell(
        content: 'X',
        width: 1,
        style: const Style(fg: UvRgb(255, 0, 0)),
      ),
    );
    r.render(buf);
    r.flush();
  ...
  test('TerminalRenderer parity: position and setPosition', () {
    final out = _TestSink();
    final r = TerminalRenderer(out, env: const ['TERM=xterm-256color']);
  ...
  test('TerminalRenderer parity: moveTo', () {
    final out = _TestSink();
    final r = TerminalRenderer(out, env: const ['TERM=xterm-256color']);
    r.moveTo(5, 3);
    r.flush();
  ...
  test('TerminalRenderer parity: writeString and write', () {
    final out = _TestSink();
    final r = TerminalRenderer(out, env: const ['TERM=xterm-256color']);
  ...
  test('TerminalRenderer parity: redraw', () {
    final out = _TestSink();
    final r = TerminalRenderer(out, env: const ['TERM=xterm-256color']);
  ...
  test('TerminalRenderer parity: capabilities', () {
    final out = _TestSink();
    final xterm = TerminalRenderer(out, env: const ['TERM=xterm-256color']);
    expect(
      xterm.capabilities & TerminalRenderer.capCha,
      TerminalRenderer.capCha,
    );

    final linux = TerminalRenderer(out, env: const ['TERM=linux']);
    expect(
      linux.capabilities & TerminalRenderer.capVpa,
      TerminalRenderer.capVpa,
    );
    expect(
      linux.capabilities & TerminalRenderer.capHpa,
      TerminalRenderer.capHpa,
    );
    expect(linux.capabilities & TerminalRenderer.capRep, 0);

    final alacritty = TerminalRenderer(out, env: const ['TERM=alacritty']);
    expect(alacritty.capabilities & TerminalRenderer.capCht, 0);

    final screen = TerminalRenderer(out, env: const ['TERM=screen']);
    expect(screen.capabilities & TerminalRenderer.capRep, 0);

    final tmux = TerminalRenderer(out, env: const ['TERM=tmux']);
    expect(
      tmux.capabilities & TerminalRenderer.capVpa,
      TerminalRenderer.capVpa,
    );
  });

  test('TerminalRenderer parity: touched count', () {
    final out = _TestSink();
    final r = TerminalRenderer(out, env: const ['TERM=xterm-256color']);
  ...
  test('TerminalRenderer parity: switch buffer', () {
    final out = _TestSink();
    final r = TerminalRenderer(out, env: const ['TERM=xterm-256color']);
  ...
  test('TerminalRenderer parity: phantom cursor', () {
    final out = _TestSink();
    final r = TerminalRenderer(out, env: const ['TERM=xterm-256color']);
    r.setColorProfile(cp.Profile.trueColor);
    r.setFullscreen(true);
  ...
  test('TerminalRenderer parity: updates (upstream frames)', () {
  ...
  test('TerminalRenderer parity: prepend one line', () {
    final out = _TestSink();
    final r = TerminalRenderer(out, env: const ['TERM=xterm-256color']);
  ...
  test('TerminalRenderer parity: enter/exit alt screen', () {
    final out = _TestSink();
    final r = TerminalRenderer(out, env: const ['TERM=xterm-256color']);
  ...
  test('TerminalRenderer parity: resize with clear does not crash', () {
    ...
    // Simulate UltravioletRenderer resize path: `erase()` then render a smaller buffer.
    r.erase();
