...
TerminalRenderer _newRenderer(_TestSink out, {List<String>? env}) {
...
void main() {
  group('TerminalRenderer parity (additional upstream cases)', () {
    ...
    test('TestRendererStyledText', () {
      ...
      final styles = <Style>[
        const Style(),
        const Style(attrs: Attr.bold),
        const Style(fg: UvRgb(255, 0, 0)),
        const Style(bg: UvRgb(0, 255, 0)),
        const Style(attrs: Attr.bold, fg: UvRgb(0, 0, 255)),
      ];
      ...
      for (var i = 0; i < styles.length && i < cellbuf.width(); i++) {
        cellbuf.setCell(
          ...
          Cell(
            content: 'U',
            width: 1,
            style: Style(underline: styles[i]),
          ),
        );
      }
      ...
      for (final p in profiles) {
        ...
        cellbuf.setCell(
          ...
          Cell(
            content: 'C',
            width: 1,
            style: const Style(fg: UvRgb(123, 234, 45)),
          ),
        );
