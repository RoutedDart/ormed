...
class TableDemoModel implements tui.Model {
  ...
  (tui.Model, tui.Cmd?) update(tui.Msg msg) {
    if (msg is tui.KeyMsg) {
      ...
      final rune = isRune ? key.runes[0] : -1;

      // Toggle focus on Esc.
      if (key.type == tui.KeyType.escape) {
        table.focused ? table.blur() : table.focus();
        return (this, null);
      }

      // Quit on q or Ctrl+C.
      if ((isRune && rune == 0x71) || (key.ctrl && rune == 0x63)) {
        return (this, tui.Cmd.quit());
      }

      // Enter prints the selected city.
      if (key.type == tui.KeyType.enter) {
        final city = table.selectedRow?[1] ?? '';
        return (this, tui.Cmd.printf("Let's go to %s!", [city]));
      }
    }
...
Future<void> main() async {
  ...
  final table = tui.TableModel(columns: columns, rows: rows, focused: true)
    ..setHeight(8)
    ..setStyles(
      tui.TableStyles(
        header: tui.TableStyles.defaults().header
            .foreground(const AnsiColor(240))
            .bold(false),
        selected: tui.TableStyles.defaults().selected
            .foreground(const AnsiColor(229))
            .background(const AnsiColor(57))
            .bold(false),
      ),
    );

  await tui.runProgram(
    TableDemoModel(table),
