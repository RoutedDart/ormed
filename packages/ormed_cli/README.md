...
dart run orm_cli:orm init

# Create a new migration file and register it in the registry
dart run orm_cli:orm make --name create_users_table

# Apply pending migrations (use --preview to show diff + SQL)
dart run orm_cli:orm apply
dart run orm_cli:orm apply --preview

# Roll back migrations (also accepts --preview)
dart run orm_cli:orm rollback --steps 1

# Show migration status
dart run orm_cli:orm status

# Run seeders
dart run orm_cli:orm seed --class DatabaseSeeder
```
...
by `orm_cli:orm init`. Add `--preview` to any migration command to see the
