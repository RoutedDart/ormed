...
class DemoContentSeeder extends Seeder {
  DemoContentSeeder(super.context);
  ...
  Future<void> run() async {
    ...
    final tags = await _ensureTags(connection, seeder);
    await _ensurePosts(connection, seeder, [admin, guest], tags);
  }
  ...
  Future<List<Tag>> _ensureTags(
    ...
    for (final name in desired) {
      await seeder.insert(Tag(name: name, createdAt: now, updatedAt: now));
    }
  ...
  Future<void> _ensurePosts(
    ...
    await seeder.insertMany(newPosts);
    ...
    if (pivotRows.isNotEmpty) {
      await seeder.insertMany(pivotRows);
    }
    if (comments.isNotEmpty) {
      await seeder.insertMany(comments);
    }
  }
}
