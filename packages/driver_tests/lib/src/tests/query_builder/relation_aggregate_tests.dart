...
void runRelationAggregateTests() {
  ormedGroup('relation aggregates', (dataSource) {
    setUp(() async {
    ...
    test(
      'loadSum calculates sum of relation column',
      () async {
    ...
    test(
      'loadSum returns 0 for empty relations',
      () async {
        // Create author with no posts
        await dataSource.repo<Author>().insertMany([
          const Author(id: 99, name: 'Lonely'),
        ]);
        final rows = await dataSource.context
            .query<Author>()
            .where('id', 99)
      ...
      () async {
        ...
        final authors = await dataSource.context
            ...
            .withSum(
              ...
              alias: 'high_views',
              constraint: (q) =>
                  q.where('views', 50, PredicateOperator.greaterThan),
            )
            .where('id', 1)
            .get();
