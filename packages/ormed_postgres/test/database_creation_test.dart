...
void main() {
  group('Postgres Database Creation', () {
    test('Postgres driver can create new database', () async {
      final url = Platform.environment['POSTGRES_URL'] ??
    ...
    test('Postgres driver can connect to newly created database', () async {
      ...
      try {
        final testDbName = 'orm_test_connect_${DateTime.now().millisecondsSinceEpoch}';
    ...
    test('Postgres driver can create databases with different names for parallel tests', () async {
      final url = Platform.environment['POSTGRES_URL'] ??
          'postgres://postgres:postgres@localhost:6543/orm_test';
      ...
      try {
        ...
        for (var i = 1; i <= 3; i++) {
          final testDbName = 'orm_test_parallel_${DateTime.now().millisecondsSinceEpoch}_$i';
