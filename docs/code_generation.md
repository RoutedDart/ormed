...
- `<model>.orm_model.json` — An intermediate artifact used by the auto-registry
  builder to discover all ORM models in your project.
...
The ORM ships two builders that can be configured via `build.yaml`:

### `ormed` Builder (Model Generator)

Generates `.orm.dart` files for each annotated model class.
...
### `orm_registry` Builder (Auto-Registry)

Aggregates all generated models into a single registry file.
...
|---------|--------|---------|-------------|
| `ormed\|orm_registry` | `output` | `lib/orm_registry.g.dart` | Path for the generated registry file |

### Disabling Auto-Registry

If you prefer manual registration or want to avoid generating `lib/orm_registry.g.dart`,
disable the registry builder in your `build.yaml`:
...
When disabled, you'll need to register models manually:

```dart
import 'package:ormed/ormed.dart';
import 'user.orm.dart';
import 'post.orm.dart';
...
You can also safely delete existing `orm_registry.g.dart` and `*.orm_model.json` files
after disabling the builder—they won't be regenerated.

### Custom Output Location

To place the registry in a different location:
...
safely add `*.orm_model.json` to your `.gitignore` since they're regenerated
during each build.

### Recommended `.gitignore`

Add these patterns to your `.gitignore` for generated files:
...
If you commit generated `.orm.dart` files (recommended for faster CI builds),
only ignore the `.orm_model.json` intermediates. If you prefer to regenerate
everything in CI, you can ignore all generated files.
...
The build system automatically generates `lib/orm_registry.g.dart` containing
all discovered ORM models. This file provides:

- `generatedOrmModelDefinitions` — A list of all model definitions
- `buildOrmRegistry()` — Creates a pre-populated `ModelRegistry`
- `registerGeneratedModels()` — Extension method to register all models at once

```dart
import 'package:your_package/orm_registry.g.dart';

// Option 1: Use the pre-built registry
final registry = buildOrmRegistry();

// Option 2: Register into existing registry
final registry = ModelRegistry()..registerGeneratedModels();

// Option 3: Use with DataSource (recommended)
final ds = DataSource(DataSourceOptions(
  driver: SqliteDriverAdapter.file('app.sqlite'),
...
You can also register models manually:
...
Models that mix in `ModelFactoryCapable` get generated factory helpers for
...
// Generate test data
final userData = Model.factory<User>()
  .seed(42)
...
The factory builder supports deterministic seeding, field overrides, custom
generators, and integration with the ORM persistence layer.

> **See [Model Factories](model_factories.md)** for complete documentation including
> custom generator providers, connection-bound helpers, testing patterns, and
> inheritance support.
