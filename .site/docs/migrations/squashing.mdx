...
- Create schema dumps intentionally for faster bootstrap
- Decide when pruning is safe for your team/release process
- Understand how dump loading interacts with migration ledger state
...
Treat pruning as a release operation, not a daily workflow.
...
## Verify Squash Safety

1. Bootstrap a fresh DB from dump in CI/staging.
2. Apply post-dump migrations successfully.
3. Confirm existing environments with non-empty ledgers still migrate normally.
