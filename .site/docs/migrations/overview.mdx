...
- You have runtime config in place ([Configuration](../getting-started/configuration))
- You have models to back your schema changes ([Defining Models](../models/defining-models))

## What You’ll Learn

- How Ormed migration formats work (Dart + SQL)
- How migration ordering and rollback semantics are enforced
- How to structure safe table creation and alteration workflows

## Step Outcome

By the end of this page, you should have:

- A migration format choice for your team (Dart, SQL, or mixed)
- A naming/order strategy that keeps migrations deterministic
- Reversible `up`/`down` implementations for new schema changes
...
Pick one default format for day-to-day changes; use the other only when it is clearly better for a specific migration.
...
1. Additive change first (nullable/defaulted columns when needed).
2. Backfill data where required.
3. Enforce stricter constraints in a follow-up migration.
4. Keep rollback path valid for each step.
