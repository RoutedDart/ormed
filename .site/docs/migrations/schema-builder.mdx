...
- How to define table schemas with Ormed’s fluent API
- How column/index/constraint helpers map to SQL backends
- How schema operations interact with table prefixes and drivers

## Step Outcome

By the end of this page, you should be able to:

- Author create/alter/drop operations with reversible intent
- Choose column/index/foreign-key helpers correctly
- Apply driver-specific schema differences intentionally
...
Create parent/reference tables before child tables that depend on them.

:::note Table prefixing
...
Use modifiers (`nullable`, defaults, indexes, comments) to make runtime behavior explicit and rollback-safe.
...
When altering populated tables, prefer additive changes first (nullable/defaulted columns), then tighten constraints in follow-up migrations.
...
Before applying risky schema changes:

```bash
dart run ormed_cli:ormed migrate --pretend
```

Check the generated SQL for column nullability, defaults, index names, and FK actions.
