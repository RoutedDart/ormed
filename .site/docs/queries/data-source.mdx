...
The `DataSource` class provides a modern, declarative API for configuring and
using the ORM. It bundles driver configuration, entity registration, connection
management, and execution middleware into a single interface. For a simpler
driver-first entrypoint, use [`OrmDatabase`](../getting-started/direct-database),
which owns a `DataSource` for you.
...
| `interceptors` | `List<QueryInterceptor>` | `[]` | Ordered middleware around query, mutation, raw SQL, schema, migration, stream, and transaction execution |
...
Interceptors are the middleware layer for policies, metrics, tracing, and
short-circuiting. They receive immutable SQL, parameters, operation metadata,
and a transaction identifier when applicable:

```dart
final ds = DataSource(DataSourceOptions(
  driver: myDriver,
  registry: ModelRegistry(),
  interceptors: [QueryTimingInterceptor()],
));
await ds.init();
```

The same list can be passed to `OrmDatabase.connect` or the direct
`SqliteDatabase`, `PostgresDatabase`, `MySqlDatabase`, and `D1Database` helpers.
Use the facade methods (`queryRaw`, `executeRaw`, `transaction`, and
`executeSchema`) when raw or schema work should enter the same pipeline. A
direct call on `db.driver` is intentionally the low-level escape hatch.

See [Observability](../guides/observability) for the built-in Dartastic
OpenTelemetry adapter and custom middleware examples.
