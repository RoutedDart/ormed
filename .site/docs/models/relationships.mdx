...
- Correct relation annotations for your model graph
- A chosen loading strategy (eager vs lazy) per endpoint/use case
- Basic manipulation flows (associate/attach/sync) working in tests
...
- `belongsTo`: this model stores the foreign key.
- `hasOne`/`hasMany`: related model stores the foreign key.
- `belongsToMany`: join/pivot table between both models.
- Polymorphic (`morph*`): one association can point to multiple model types.
...
Use eager loading for list/detail endpoints that always need related data. Use lazy loading for optional or rarely accessed relations.
...
## Verify Relation Behavior

1. Add tests for at least one eager-load path.
2. Add tests for one relation mutation path (`associate` / `attach` / `sync`).
3. Confirm no N+1 behavior in hot list endpoints.
