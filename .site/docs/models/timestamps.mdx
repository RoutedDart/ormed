...
1. Insert model and confirm both `created_at` + `updated_at` are set.
2. Update model and confirm only `updated_at` changes.
3. Validate timezone behavior (`Timestamps` vs `TimestampsTZ`) in your runtime.
